import React, { useMemo, useRef, useEffect, useState, useLayoutEffect, cloneElement, createElement, memo, useCallback, forwardRef, useImperativeHandle } from 'react';
...
function SimpleCellFormatter({ row, column }) {
...
function isSelectedCellEditable({ selectedPosition, columns, rows, onCheckCellIsEditable, isGroupRow }) {
...
const HeaderRow$1 = memo(HeaderRow);
...
const FilterRow$1 = memo(FilterRow);
...
const Cell$1 = memo(forwardRef(Cell));
...
function EditorPortal({ target, children }) {
...
function EditorContainer({ rowIdx, column, row, rowHeight, left, top, onCommit, onCommitCancel, scrollLeft, scrollTop, firstEditorKeyPress: key }) {
    ...
    return (React.createElement("div", { className: className, style: { height: rowHeight, width: column.width, left, top }, onClickCapture: onClickCapture, onKeyDown: onKeyDown, onContextMenu: preventDefault }, createEditor()));
}

function Editor2Container({ row, column, onRowChange, editorPortalTarget, ...props }) {
    var _a;
    const onClickCapture = useClickOutside(() => onRowChange(row, true));
    if (column.editor2 === undefined)
        return null;
    const editor = (React.createElement("div", { className: "rdg-editor-container2", onClickCapture: onClickCapture },
...
function EditCell({ className, column, row, rowIdx, editorPortalTarget, editorContainerProps, editor2Props, onKeyDown, ...props }, ref) {
    ...
    function getCellContent() {
        ...
        if (column.editor2 !== undefined) {
            return (React.createElement(Editor2Container, Object.assign({}, editor2Props, { editorPortalTarget: editorPortalTarget, rowIdx: rowIdx, column: column, left: gridLeft, top: gridTop })));
        }
...
const EditCell$1 = forwardRef(EditCell);
...
function Row({ cellRenderer: CellRenderer = Cell$1, className, eventBus, rowIdx, isRowSelected, copiedCellIdx, draggedOverCellIdx, row, viewportColumns, selectedCellProps, onRowClick, rowClass, setDraggedOverRowIdx, onMouseEnter, top, 'aria-rowindex': ariaRowIndex, 'aria-selected': ariaSelected, ...props }, ref) {
    ...
    return (React.createElement("div", Object.assign({ role: "row", "aria-rowindex": ariaRowIndex, "aria-selected": ariaSelected, ref: ref, className: className, onMouseEnter: wrapEvent(handleDragEnter, onMouseEnter), style: { top } }, props), viewportColumns.map(column => {
        ...
        if ((selectedCellProps === null || selectedCellProps === void 0 ? void 0 : selectedCellProps.mode) === 'EDIT' && isCellSelected) {
            return (React.createElement(EditCell$1, { key: column.key, rowIdx: rowIdx, column: column, row: row, onKeyDown: selectedCellProps.onKeyDown, editorPortalTarget: selectedCellProps.editorPortalTarget, editorContainerProps: selectedCellProps.editorContainerProps, editor2Props: selectedCellProps.editor2Props }));
        }
...
const Row$1 = memo(forwardRef(Row));
...
const GroupCell$1 = memo(GroupCell);
...
const GroupRowRenderer = memo(GroupedRow);
...
const SummaryCell$1 = memo(SummaryCell);
...
const SummaryRow$1 = memo(SummaryRow);
...
// Toggles and modes
enableFilters = false, enableCellCopyPaste = false, enableCellDragAndDrop = false, cellNavigationMode = 'NONE', 
// Miscellaneous
...
'aria-label': ariaLabel, 'aria-labelledby': ariaLabelledBy, 'aria-describedby': ariaDescribedBy }, ref) {
    ...
    return (React.createElement("div", { role: hasGroups ? 'treegrid' : 'grid', "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, "aria-describedby": ariaDescribedBy, "aria-multiselectable": isSelectable ? true : undefined, "aria-colcount": columns.length, "aria-rowcount": headerRowsCount + rowsCount + summaryRowsCount, className: clsx('rdg', { 'rdg-viewport-dragging': isDragging }, className), style: {
        ...
        React.createElement(HeaderRow$1, { rowKey: rowKey, rows: rawRows, columns: viewportColumns, onColumnResize: handleColumnResize, allRowsSelected: (selectedRows === null || selectedRows === void 0 ? void 0 : selectedRows.size) === rawRows.length, onSelectedRowsChange: onSelectedRowsChange, sortColumn: sortColumn, sortDirection: sortDirection, onSort: onSort }),
        enableFilters && (React.createElement(FilterRow$1, { columns: viewportColumns, filters: filters, onFiltersChange: onFiltersChange })),
...
const DataGrid$1 = forwardRef(DataGrid);

export default DataGrid$1;
export { Cell$1 as Cell, Row$1 as Row, SELECT_COLUMN_KEY, SelectCellFormatter, SelectColumn, SimpleCellFormatter, SimpleTextEditor, ToggleGroupFormatter, ValueFormatter };
