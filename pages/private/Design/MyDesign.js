...
class MyDesign extends React.Component{
    ...
    render(){
        return(
            <>
                <div><h1>My Designs</h1></div>
                <nav className="navbar navbar-dark bg-dark shadow mb-5">
                    <div nav className="nav-left">
                    <PhaseDiv>
                        <b>Cards Valide</b>
                        {this.data.map((data) => {if(data.expired == false) return <BaseCard key={data.nome} name={data.nome} description={data.descrizione}/>})}
                    </PhaseDiv>
                    <PhaseDiv>
                        <b>Cards Scadute</b>
                        {this.data.map((data) => {if(data.expired == true) return <BaseCard key={data.nome} name={data.nome} description={data.descrizione}/>})}   
                    </PhaseDiv>
                    </div>
                </nav>
            </>
